...
You can pause an agent so it stops executing on its schedule, then resume it later — or stop it entirely. 
...
Important: This does not prevent it from being executed, but it tells the execution to stop al work so it dies almost as quickly as it wakes up. 
...
If an agent is no longer needed, stop it and remove it. It stays in your records for the audit trail but no longer runs or appears as active on your org chart.
